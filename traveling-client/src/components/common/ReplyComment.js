...
function ReplyComment ( { scm, refresh, setRefresh } )
{
    ...
    {
        axios.get( "http://localhost:8080/api/like/getByCommentId/" + scm?.commentId,
            {
                headers: {
    ...
    const handleComment = ( e ) =>
    {
        let commentData = {
            "userId": loginUserId,
            "postId": scm?.postId,
            "parentId": scm?.parentId ? scm?.parentId : scm?.commentId,
            "replyId": scm?.commentId,
            "text": comment
        };
        let jsonBlob = ( obj ) =>
        {
    ...
    return (
        <React.Fragment>
            <DeleteCommentModal data={ scm } refresh={ refresh } setRefresh={ setRefresh } />
            <li className="mb-2 ml-5">
                <div className="d-flex flex-wrap">
                    <div className="user-img">
                        <img
                            src={ scm?.user?.avatarSrc }
                            alt="user-img"
                            className="avatar-40 rounded-circle img-fluid"
                        />
                    </div>
                    <div className="comment-data-block ml-3">
                        <Link to={ "/profile/" + scm?.userId } className="h6">{ scm?.user?.name } </Link>trả lời  <Link to={ "/profile/" + scm?.replyUser?.userId } className="h6">{ scm?.replyUser?.name }</Link>
                        <span className="mb-0"> -- { convertDate( scm?.regDate ) }</span>
                        { scm?.isDelete === SystemConst.FLG_ON ? <p className="font-italic"> Bình luận đã bị xóa</p> :
                            <>
                                { scm?.userId === loginUserId && <span className="btn m-0 p-0 ml-3 text-danger" data-toggle="modal" data-target={ "#delete-comment-modal-" + scm?.commentId }>
                                    <i class="las la-trash"></i>Xóa</span> }

                                <p className="text-justify mt-1 mb-1">{ scm?.text }</p>
                                { scm?.images != null &&
                                    <img style={ {
                                        height: "180px",
                                        objectFit: "contain"
                                    } } src={ scm?.images?.path } alt="post-img" className="img-fluid rounded"></img>
                                }
                                <div className="d-flex flex-wrap align-items-center comment-activity">
                                    ...
                                    {
                                        handleLikeComment( scm?.commentId )
